...
{
	struct parser_context* ctx = new_parser_with_io((new_repl_0(argv[1])));;
	struct tree_descr* internal_ex = tree_descr(ctx);
	
	add_cpts_portion("json.gr", ctx, internal_ex);
	struct attnode* json = fixate_cpts(ctx, internal_ex, "root");
